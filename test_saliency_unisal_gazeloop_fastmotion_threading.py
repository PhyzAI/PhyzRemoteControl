...
def apply_foveal_suppression(rgb_image, ior_list, current_time, ior_duration=4.0, attenuation=0.25):
    ...
    for ior_x, ior_y, timestamp, ior_radius in ior_list:
        if (current_time - timestamp) < ior_duration:
            ...
            ksize = int(ior_radius * 0.5) # was 0.5
            if ksize % 2 == 0:
                ksize += 1
            ksize = max(3, ksize)
...
def suppress_heatmap_around_points(saliency_map, points, radius=180, suppression_factor=0.2):
    """
...
def apply_saliency_suppression_and_extract_targets(smoothed_map, gaze_manager, current_time, orig_w, orig_h, attenuation=0.70):
    """
...
# Initialize Threaded Camera Stream (0 is default webcam index)
cap = ThreadedWebcam(src=0)
# Allow camera thread a moment to warm up
time.sleep(0.5)
...
with torch.no_grad():
    while True:
        ...
        # ------------------------------------------------------------------
        # Apply Frame Scaling (50% size reduction)
        # ------------------------------------------------------------------
        ...
        # 1. Process Motion Memory and Detect Spikes (Now running on lighter 0.5x frame!)
        ...
        # Output frame is already lightweight (0.5x), so we display directly
        combined = overlay
