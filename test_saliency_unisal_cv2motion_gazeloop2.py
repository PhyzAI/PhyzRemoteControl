...
class PhyzyGazeManager:
    def __init__(self, primary_dwell_range=(2.0, 4.0), secondary_dwell_range=(0.8, 1.8), ior_duration=4.0):
        self.current_gaze = None        # (x, y)
        self.current_radius = 40        # Dynamic radius
        self.gaze_start_time = 0.0
        self.current_dwell_time = 2.0   # Active target's sampled dwell limit
        
        # Ranges for randomized gaze locks
        self.primary_dwell_range = primary_dwell_range      # Focus targets
        self.secondary_dwell_range = secondary_dwell_range  # Quick glances
        
        # IOR list stores tuples: (x, y, timestamp, radius)
        self.ior_list = []
        self.ior_duration = ior_duration

    def _sample_dwell_time(self, is_primary=True):
        """Samples a randomized dwell duration from uniform or normal distribution."""
        dwell_min, dwell_max = self.primary_dwell_range if is_primary else self.secondary_dwell_range
        # Uniform sampling within range (or use random.gauss for bell curve distribution)
        return random.uniform(dwell_min, dwell_max)

    def update(self, candidates, current_time):
        """
        candidates: List of (x, y, score, radius) tuples sorted by saliency priority
        """
        # Clean expired IOR entries
        ...
        if self.current_gaze is None:
            ...
            self.current_dwell_time = self._sample_dwell_time(is_primary=True)
        else:
            time_spent = current_time - self.gaze_start_time

            # Check if active randomized dwell timer has expired
            if time_spent >= self.current_dwell_time:
                curr_x, curr_y = self.current_gaze
                # Push active gaze to Inhibition of Return list
                self.ior_list.append((curr_x, curr_y, current_time, self.current_radius))
                
                # Switch gaze to top unsuppressed candidate
                self.current_gaze = (best_x, best_y)
                self.current_radius = best_radius
                self.gaze_start_time = current_time
                
                # Sample NEW random dwell duration for this fresh gaze lock!
                # If target score is high (>0.7), consider it a primary interest point
                is_primary_target = (best_score >= 0.65)
                self.current_dwell_time = self._sample_dwell_time(is_primary=is_primary_target)
                
            else:
                # Smooth pursuit tracking while current dwell timer is active
...
def apply_foveal_suppression(rgb_image, ior_list, current_time, ior_duration=4.0, attenuation=0.25):
    """
...
def process_motion_memory(frame, prev_gray, motion_memory, decay_rate=0.9):
    """
    Computes frame-by-frame differential motion and updates decaying motion memory.
    """
    gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    gray_resized = cv2.resize(gray_frame, (384, 288))

    if prev_gray is None:
        return np.zeros((288, 384), dtype=np.float32), gray_resized, np.zeros((288, 384), dtype=np.float32)
    ...
    current_motion = motion_blur / max_diff if max_diff > 30.0 else np.zeros((288, 384), dtype=np.float32)

    # Decay previous memory and accumulate new motion
    updated_memory = np.maximum(motion_memory * decay_rate, current_motion)
    return current_motion, gray_resized, updated_memory
...
with torch.no_grad():
    while True:
        ...
        # 1. Process Motion Memory
        _, prev_gray, motion_memory = process_motion_memory(frame, prev_gray, motion_memory, decay_rate)

        # 2. Run UNISAL Static Saliency (With Input Foveal Suppression)
        static_sal = run_unisal_model(model, frame, gaze_manager, current_time, device)

        # 3. Blend Motion & Apply Temporal EMA Smoothing
        ...
        # 4. Extract Targets (Only past IOR locations are suppressed)
        ...
        # 5. Update Phyzy Gaze State Engine
